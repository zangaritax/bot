        ...
        if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
          ...
          if (m.text && user.banned && !isROwner) {
            ...
            if (user.bannedMessageCount < 3) {
              ...
              const messageText = `╔═════════════════════════╗
 ❰ ⚠️ ❱ *أوبا! انت متبند ياحب يا نجم!* ❰ ⚠️ ❱
—◉ *دي الملاحظة رقم ${messageNumber}/3 (الإجمالي: 3)*
—◉ ${user.bannedReason ? `\n*السبب:* ${user.bannedReason}` : '*السبب:* مش معروف'}
—◉ *لو حاسس إن ده غلط ومعاك أدلة، ممكن تكلم صاحب البوت وتتظلم على الإيقاف.*
—◉ *لو حابب تستأنف:* wa.me/+99440 560 81 55
╚═════════════════════════╝`.trim();
              m.reply(messageText);
    ...
    case 'add':
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          ...
          const groupName = groupMetadata.subject || 'غير معروف';
          const groupId = id;
          const participantsList = groupMetadata.participants || [];
          const owner = groupMetadata.owner;
          const ownerMention = owner ? `@${owner.split('@')[0]}` : 'غير معروف';
          const isBotMember = participantsList.some(p => p.id === mconn?.conn?.user?.jid) ? 'نعم' : 'لا';
          const botStayDuration = global.botStayDuration?.[groupId] || Infinity;
          
          const formatEndDate = (days) => {
            if (days === Infinity) return '🌀 غير محدد';
          ...
          const adminMentions = admins.map(a => `@${a.id.split('@')[0]}`).join(', ') || 'لا يوجد';
...
┇🏮📌║الاسم:〘${groupName}〙
┇🏮🆔║ID الجروب:〘${groupId}〙
┇🏮⏳║مدة بقاء البوت:〘${botStayDuration === Infinity ? '🌀 غير محدد' : botStayDuration + ' يوم'}〙
┇🏮📅║البوت سيغادر في:〘${formattedEndDate}〙
┇🏮🤖║أنا مشارك:〘${isBotMember}〙
┇🏮👥║عدد الأعضاء:〘${totalMembers}〙
┇🏮👤║منشئ الجروب:〘${ownerMention}〙
...
╯✯≼══━━﹂⛩️﹁━━══≽✯`.trim();
            ...
            const welcomeMsg = `
*⊹‏⊱≼━⌬〔 NANO-BOT 〕⌬━≽⊰⊹*
● تسجيل دخول من* *༺ @${userNumber} ༻*
*𓆩نورت يا حب𓆪*
✯≼══━━﹂⛩️﹁━━══≽✯

● *نرحب بك في جروب:* ☾ ${groupName} ☽
● *شوف الوصف يا حب*
...
*⊹‏⊱≼━⌬〔 NANO-BOT 〕⌬━≽⊰⊹*`.trim();
            ...
            // إرسال الصوت الترحيبي للأعضاء العاديين فقط
